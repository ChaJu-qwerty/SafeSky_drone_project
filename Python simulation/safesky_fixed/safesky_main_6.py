...
# FIX: WP_ACCEPT aumentado 0.60→1.00m — con obs_push activo el dron no puede
# acercarse más de ~0.8m al WP sin que obs_push lo desvíe; 0.60m era inalcanzable
WP_ACCEPT      = 1.00
V_MAX_NAV      = 2.0
TRAIL_LEN      = 120
...
class DroneAgent:
    ...
    def __init__(self, did, start, goal_or_wps, params: QuadParams,
        ...
        if mode == 'ab':
            ...
            self.takeoff_alt = max(first_wp_z, init[2] + 2)
...
class SafeSkyApp:
    ...
    def _export(self, fmt='gif'):
        ...
        # ── Exportar frame a frame ────────────────────────────
        try:
            if fp.lower().endswith('.gif'):
                # GIF con PIL
                # FIX: no usar Image.open(buf) dentro del loop — PIL cierra
                # el BytesIO al liberar la imagen. Usar np.frombuffer en su lugar.
                try:
                ...
                for idx, fi in enumerate(frames_idx):
                    ...
                    buf.seek(0)
                    # FIX: leer PNG completo a memoria antes de cerrar BytesIO
                    img = Image.open(buf)
                    img.load()                # fuerza lectura completa
                    # Convertir a paleta P para GIF optimizado
                    ...
                    lbl_frame.config(text=f"Frame {idx+1} / {total_f}  ({pct}%)")
                    lbl_eta.config(
                        text=f"⏱ {elapsed:.0f}s  |  ETA: {eta:.0f}s  —  no cerrar")
                    self._status_var.set(f"GIF: {pct}%  ({idx+1}/{total_f})")
                    prog_win.update(); self.root.update()

                lbl_frame.config(text="Guardando GIF en disco...")
                prog_win.update()
                dur_ms = max(60, int(1000 / 12))   # 12 fps
                # Convertir a paleta P para tamaño de archivo reducido
                frames_p = [f.quantize(colors=256, method=2) for f in frames_pil]
                frames_p[0].save(
                    fp, save_all=True,
                ...
                with writer_mp4.saving(fig_ex, fp, dpi=90):
                    for idx, fi in enumerate(frames_idx):
                        ...
                        lbl_eta.config(
                            text=f"⏱ {elapsed:.0f}s  |  ETA: {eta:.0f}s  —  no cerrar")
                        self._status_var.set(f"MP4: {pct}%  ({idx+1}/{total_f})")
                        prog_win.update(); self.root.update()
