...
NOVEDADES v9:
  * Objeto intrusor externo: cruza la trayectoria del enjambre a velocidad
    configurable; botón "💥 LANZAR INTRUSO" durante la simulación.
...
DT        = 0.05
DT_ANIM   = 60
SUCCESS_M      = 0.50   # radio de llegada [m] — aumentado de 0.35→0.50 para ser alcanzable
                        # bajo viento real (oscilación media ~0.4m con sigma=0.60)
WP_ACCEPT      = 0.60   # radio de aceptación de waypoints intermedios
V_MAX_NAV      = 2.0
TRAIL_LEN      = 120
HOVER_STEPS    = 150     # pasos CONSECUTIVOS dentro de SUCCESS_M para declarar arrived
                        # = 1 segundo (DT=0.05s). Antes era 120 (6s) → IMPOSIBLE con viento
HOVER_TOLERANCE= 8      # pasos fuera del radio que NO resetean el progreso (más generoso)
...
GUST_MAG    = 8.0   # m/s lateral — genera F≈1.9N → a≈1.9m/s², claramente visible
...
class Simulation:
    def __init__(self, n_drones, n_obs, wind_alpha, wind_sigma,
        ...
        self.orca         = ORCAPlanner(radius=ORCA_RADIUS, tau=4.0, v_max=V_MAX_NAV)
        ...
        if self.mission_mode == 'ab':
            ...
            for i in range(n):
                ...
                for _ in range(12):
                    ...
                    if all(min(o.dist_surface(p) for o in self.obstacles) >= 0.35
                           for p in ascent):
                        break
    ...
    def step(self):
        ...
        for d in self.drones:
            if d.arrived or not d.takeoff_done:
                ...
                dv = d.current_wp() - d.state.pos
                dn = np.linalg.norm(dv)
                v_prefs.append(dv/dn * min(V_MAX_NAV, dn) if dn > 0.01 else np.zeros(3))
...
class SafeSkyApp:
    ...
    def _populate_kpi_tab(self, kpi: MissionKPI, mission_mode: str):
        ...
        # Usar constantes calculadas: D_COLLISION_PHYSICAL = 2×R_DRONE = 0.690m
        ...
        if any_alert:
            ...
            if coll_obs_phys:
                msg = "  🔴 COLISIÓN CON OBSTÁCULO: " + ", ".join(
            ...
            if coll_drone:
                msg = "  🟠 COLISIÓN ENTRE DRONES: " + ", ".join(
                    ...
                    f"[brazo {R_DRONE_BODY:.3f}m × 2 = hélices se tocan])"
                    for d in coll_drone)
        ...
        items_g = [
            ...
            ("Viento α",         f"{kpi.wind_alpha:.2f}",        CC),
            ("Viento σ",         f"{kpi.wind_sigma:.2f}",        CC),
        ]
    ...
    def _populate_planes_tab(self, sim):
        ...
        #   D_COLLISION_PHYSICAL = 2 × R_DRONE = 2 × (l_arm + r_prop) = 0.690 m
        ...
        # 1) Colisión física: 2 × (brazo + hélice)
        ax_dd.axhline(D_COLLISION_PHYSICAL, color=CR, lw=1.2, ls=':', alpha=0.9)
        ax_dd.text(0.01, D_COLLISION_PHYSICAL + 0.04,
                   f"  COLISIÓN FÍSICA = 2×(l_arm+r_prop) = 2×({R_DRONE_BODY:.3f}) = {D_COLLISION_PHYSICAL:.3f}m",
                   color=CR, fontsize=6,
        ...
        for t_c, di, dj, d_c in collision_events:
            ...
            if idx_c < len(t1) and idx_c < len(t2):
                ...
                ax_xz.text(mid_xz[0], mid_xz[1]+0.3,
                           f"⚠", color=CR, fontsize=9, ha='center', zorder=13)
        ...
        if collision_events:
            items_lg.append(("", CW))
            items_lg.append(("⚠ EVENTOS DE COLISIÓN:", CR))
            for t_c, di, dj, d_c in collision_events:
                items_lg.append((f"  D{di}-D{dj}: {d_c:.3f}m @ t={t_c:.1f}s", CR))
        else:
            items_lg.append(("", CW))
            items_lg.append(("✓ Sin colisiones físicas", CG))
        y = 0.98; dy = 0.075
    ...
    def _build_perf_tab(self):
        ...
        tk.Label(self._perf_content,
                 text="\n\n⚡  Las gráficas de rendimiento aparecerán al completar la misión.",
                 bg=CB, fg=CDG, font=('Segoe UI', 12)).pack(expand=True)
        self._perf_fig    = None
    ...
    def _populate_perf_tab(self, sim):
        ...
        for i, d in enumerate(sim.drones):
            ...
            ok   = "✓" if kpi.drones[i].success else "✗"
            rows.append(
                f"D{d.id+1}  {v_avgs[i]:>5.2f}  {ve:>6.3f}"
                f"  {L_reals[i]:>6.1f}  {eff:>5.1f}%  {ok}")
        rows += [sep_row,
                 "ve_avg = err vel tracking",
                 f"α_wind = {sim.wind_alpha:.2f}",
                 f"σ_wind = {sim.wind_sigma:.2f}",
                 f"dt = {DT} s",
    ...
    def _build_controls(self):
        ...
        # ── Modo de misión ────────────────────────────
        tk.Label(P, text="🎯  MODO DE MISIÓN", bg=CP, fg=CC, font=FONT_H).pack(**pad)
        ...
        tk.Label(P, text="σ=0.60→brisa (±1.1 m/s)  σ=1.0→viento (±1.9 m/s)  σ=1.8→ráfaga",
        ...
        for lbl, var, lo, hi in [
            ("α — persistencia:", self._var_alpha, 0.0, 0.98),
            ("σ — intensidad:",   self._var_sigma, 0.0, 2.00)]:
        ...
        self._btn_gust  = tk.Button(P, text="🌪  RÁFAGA SÚBITA",**bst, command=self._trigger_gust)
        self._btn_intr  = tk.Button(P, text="💥  LANZAR INTRUSO",**bst, command=self._launch_intruder)
        self._btn_gif   = tk.Button(P, text="🎞  EXPORTAR GIF", **bst, command=lambda: self._export('gif'))
        self._btn_mp4   = tk.Button(P, text="🎬  EXPORTAR MP4", **bst, command=lambda: self._export('mp4'))
        for b in [self._btn_start, self._btn_pause, self._btn_reset,
                  self._btn_gust, self._btn_intr,
                  self._btn_gif, self._btn_mp4]:
            b.pack(padx=10, pady=2)
        self._btn_pause['state'] = 'disabled'
        self._btn_gust['state']  = 'disabled'
        self._btn_intr['state']  = 'disabled'
        self._btn_gif['state']   = 'disabled'
        self._btn_mp4['state']   = 'disabled'
    ...
    def _init_artists(self):
        ...
        self._ang_lines = []
        for col, lbl in zip([CR, CL, CG], ['φ roll', 'θ pitch', 'ψ yaw']):
    ...
    def _anim_loop(self):
        def animate(frame):
            ...
            if sim.all_arrived() and not self._mission_done:
                ...
                self._btn_gif['state'] = 'normal'
                self._btn_mp4['state'] = 'normal'
    ...
    def _update_artists(self):
        ...
        if d0.hist_wind:
            ...
            wm = np.linalg.norm(w)
            # Cuadrícula 3×3×2 = 18 puntos en el interior del volumen
            grid_pts = [(x, y, z)
                        for x in [4., 10., 16.]
                        for y in [4., 10., 16.]
                        for z in [4., 12.]]
    ...
    def _update_kpi_live(self):
        ...
        for d in sim.drones:
            ...
            st   = "✓LLEGÓ" if d.arrived else f"{dist:.2f}m"
        ...
        rows += ["─"*34,
                 ...
                 f"α={sim.wind_alpha:.2f}  σ={sim.wind_sigma:.2f}",
    ...
    #  EXPORTACIÓN GIF / MP4
    # ─────────────────────────────────────────────
    def _export(self, fmt='gif'):
        if self.sim is None: return
        ext = 'gif' if fmt == 'gif' else 'mp4'
        fp  = filedialog.asksaveasfilename(
            defaultextension=f'.{ext}',
            filetypes=([('GIF animado','*.gif')] if fmt=='gif'
                       else [('Video MP4','*.mp4'),('GIF animado','*.gif')]),
            initialfile=f'safesky_{int(time.time())}.{ext}',
            title=f'Guardar animación como {fmt.upper()}')
        if not fp: return

        # Si pide MP4, verificar ffmpeg
        if fp.lower().endswith('.mp4'):
            try:
                import subprocess
                r2 = subprocess.run(['ffmpeg','-version'],
                                    capture_output=True, timeout=5)
                has_ff = r2.returncode == 0
            except Exception:
                has_ff = False
            if not has_ff:
                if messagebox.askyesno(
                        "ffmpeg no encontrado",
                        "ffmpeg no está instalado.\n¿Exportar como GIF en su lugar?"):
                    fp = fp[:-4] + '.gif'
                else:
                    return
        ...
        self._status_var.set("📡 Fase 1/2: Simulando física..."); self.root.update()

        sim_ex = Simulation(n_dr, n_obs, alpha, sigma, obs_mode, miss_mode,
                            drone_order=self._var_order.get(),
                            goal_side=self._var_goal_side.get())
        all_pos    = [[sim_ex.drones[i].state.pos.copy()] for i in range(n_dr)]
        all_angles = [[] for _ in range(n_dr)]
        all_wind   = [[] for _ in range(n_dr)]
        all_obs_c  = [[o.center.copy() for o in sim_ex.obstacles]]
        t_hist     = [0.]
        MAX_SIM    = int(60.0 / DT)   # máx 60 s de simulación

        for s in range(MAX_SIM):
            if s % 20 == 0:   # actualizar cada 20 pasos (~1 s simulado)
                pct = int(s / MAX_SIM * 100)
                t_s = s * DT
                self._status_var.set(
                    f"📡 Fase 1/2: Simulando t={t_s:.1f}s  ({pct}%)"); self.root.update()
            sim_ex.step()
            for i, d in enumerate(sim_ex.drones):
                all_pos[i].append(d.state.pos.copy())
                all_angles[i].append(d.state.angles.copy())
                if d.hist_wind: all_wind[i].append(d.hist_wind[-1].copy())
            all_obs_c.append([o.center.copy() for o in sim_ex.obstacles])
            t_hist.append(sim_ex.t)
            if sim_ex.all_arrived(): break

        NFRAMES  = len(t_hist)
        MAX_FRAMES = 120   # 120 frames = ~10s GIF a 12fps, render ~25s
        SKIP     = max(1, NFRAMES // MAX_FRAMES)
        frames_idx = list(range(0, NFRAMES, SKIP))
        total_f    = len(frames_idx)
        t_sim_total = t_hist[-1]

        self._status_var.set(
            f"🎨 Fase 2/2: Renderizando {total_f} frames..."); self.root.update()

        # ── Figura de exportación ─────────────────────────────────
        # Resolución reducida de elipsoides: 8×6 en lugar de 12×9
        # Reduce polígonos 3D de 108 a 48 por obstáculo → ~2× más rápido
        u_s = np.linspace(0, 2*np.pi, 8)
        v_s = np.linspace(0, np.pi,   6)
        fig_ex = plt.figure(figsize=(12, 7), facecolor=CB)
        fig_ex.suptitle(
            f"SafeSky — Enjambre 6-DOF  (Euler-Lagrange + DJI F450)"
            f"  |  t_sim={t_sim_total:.1f}s  |  {n_dr} drones  {n_obs} obstáculos",
            color=CW, fontsize=10, fontweight='bold')
        gs_ex = gridspec.GridSpec(2, 3, fig_ex,
                                  left=0.04, right=0.98,
                                  top=0.94, bottom=0.06,
                                  hspace=0.38, wspace=0.30)
        ax3  = fig_ex.add_subplot(gs_ex[:, 0:2], projection='3d')
        ax_a = fig_ex.add_subplot(gs_ex[0, 2])
        ax_k = fig_ex.add_subplot(gs_ex[1, 2])

        def _render_frame(fi):
            """Renderiza un frame y retorna los ejes limpios."""
            ax3.cla()
            ...
            for p in [ax3.xaxis.pane, ax3.yaxis.pane, ax3.zaxis.pane]:
                p.fill = False
            # Obstáculos
            obs_centers = all_obs_c[min(fi, len(all_obs_c)-1)]
            for j, o in enumerate(sim_ex.obstacles):
                c  = np.array(obs_centers[j])
            ...
            for i, d in enumerate(sim_ex.drones):
                ...
                if len(tr) > 1:
                    ax3.plot(tr[:,0], tr[:,1], tr[:,2],
                             color=col, lw=1.5, alpha=0.65)
                pos_fi = np.array(all_pos[i][min(fi, len(all_pos[i])-1)])
                if all_angles[i]:
                    aix = min(fi, len(all_angles[i])-1)
                    st2 = DroneState(pos=pos_fi,
                                     angles=np.array(all_angles[i][aix]))
                ...
                if all_wind[i] and fi < len(all_wind[i]):
                    ...
                    if wm > 0.05:
                        ...
                        ax3.quiver(pos_fi[0], pos_fi[1], pos_fi[2],
                                   wu, wv, ww2,
                                   length=max(wm*0.55, 0.25),
                                   color='#A8FF78', alpha=0.85,
                                   linewidth=1.2, arrow_length_ratio=0.35)
            ax3.set_title(f"t = {t_hist[min(fi,len(t_hist)-1)]:.1f} s",
                          color=CW, fontsize=8)
            # Ángulos
            ...
            if all_angles[0] and fi > 0:
                arr = np.array(all_angles[0][:fi+1])
                ts  = t_hist[:len(arr)]
            ...
            lines_k = []
            for ii, dd in enumerate(sim_ex.drones):
                ...
                lines_k.append(
                    f"D{dd.id+1}: {'✓ OK' if dd.arrived else f'{d2g:.1f}m'}")
        ...
        prog_win.title("Exportando animación...")
        prog_win.configure(bg=CP)
        prog_win.resizable(False, False)
        self.root.update_idletasks()
        x = self.root.winfo_x() + self.root.winfo_width()//2  - 260
        y = self.root.winfo_y() + self.root.winfo_height()//2 - 90
        prog_win.geometry(f"520x195+{x}+{y}")
        prog_win.lift(); prog_win.attributes('-topmost', True)

        fmt_icon = '🎞' if fp.lower().endswith('.gif') else '🎬'
        tk.Label(prog_win,
                 text=f"{fmt_icon}  Exportando  {os.path.basename(fp)}",
                 bg=CP, fg=CW, font=('Segoe UI', 10, 'bold')).pack(pady=(12, 2))

        # Línea de fase — muestra "Fase 2/2: Renderizando frame X/N"
        lbl_fase = tk.Label(prog_win,
                            text=f"🎨 Fase 2/2: Renderizando  {total_f} frames  "
                                 f"(sim={t_sim_total:.1f}s · {n_dr} drones · {n_obs} obs)",
                            bg=CP, fg='#AAB7B8', font=('Segoe UI', 8))
        lbl_fase.pack()

        lbl_frame = tk.Label(prog_win, text="Preparando render...",
                             bg=CP, fg=CY, font=FONT_M)
        lbl_frame.pack()

        style_pb = ttk.Style()
        style_pb.theme_use('default')
        style_pb.configure('Export.Horizontal.TProgressbar',
                           troughcolor='#1A2940', background=CC,
                           darkcolor=CC, lightcolor=CC, bordercolor=CP)
        prog_bar = ttk.Progressbar(prog_win, orient='horizontal',
                                   length=480, mode='determinate',
                                   style='Export.Horizontal.TProgressbar')
        prog_bar.pack(padx=20, pady=5)
        prog_bar['maximum'] = total_f

        lbl_eta = tk.Label(prog_win, text="",
                           bg=CP, fg=CDG, font=('Segoe UI', 8))
        lbl_eta.pack()

        # Tiempo estimado por frame (medido antes)
        MS_PER_FRAME = 200   # ms — estimado conservador por frame 3D
        eta_total = total_f * MS_PER_FRAME / 1000
        lbl_eta.config(
            text=f"⏱ Tiempo estimado total: ~{eta_total:.0f}s  "
                 f"({eta_total/60:.1f} min)  —  no cerrar esta ventana")
        prog_win.update()

        t0_exp = time.perf_counter()

        # ── Exportar frame a frame (sin ani.save) ────────────────
        # Esto permite actualizar la barra de progreso en CADA frame
        # y evita que la GUI se congele.
        try:
            if fp.lower().endswith('.gif'):
                # GIF: acumular imágenes PIL frame a frame
                ...
                for idx, fi in enumerate(frames_idx):
                    _render_frame(fi)
                    buf = _io.BytesIO()
                    fig_ex.savefig(buf, format='png', dpi=85,
                                   facecolor=CB, bbox_inches='tight')
                    buf.seek(0)
                    frames_pil.append(Image.open(buf).copy())

                    # Actualizar progreso en CADA frame
                    pct     = int((idx+1) / total_f * 100)
                    elapsed = time.perf_counter() - t0_exp
                    eta     = (elapsed / max(idx+1, 1)) * (total_f - idx - 1)
                    prog_bar['value'] = idx + 1
                    lbl_frame.config(
                        text=f"Frame {idx+1} / {total_f}  ({pct}%)")
                    lbl_eta.config(
                        text=f"⏱ {elapsed:.0f}s transcurridos  |  "
                             f"ETA: {eta:.0f}s  —  no cerrar esta ventana")
                    self._status_var.set(
                        f"Exportando GIF: {pct}%  ({idx+1}/{total_f})")
                    prog_win.update(); self.root.update()

                # Guardar GIF
                lbl_frame.config(text=f"Guardando GIF en disco...")
                lbl_eta.config(text=fp)
                prog_win.update()
                dur_ms = max(60, int(1000 / 12))   # 12 fps
                frames_pil[0].save(
                    fp, save_all=True,
                ...
                # MP4: usar FFMpegWriter directamente guardando frame a frame
                writer_mp4 = mplanim.FFMpegWriter(
                    fps=20, bitrate=2500,
                    extra_args=['-vcodec','libx264','-pix_fmt','yuv420p'])
                with writer_mp4.saving(fig_ex, fp, dpi=90):
                    for idx, fi in enumerate(frames_idx):
                        _render_frame(fi)
                        ...
                        lbl_eta.config(
                            text=f"⏱ {elapsed:.0f}s transcurridos  |  "
                                 f"ETA: {eta:.0f}s  —  no cerrar esta ventana")
                        self._status_var.set(
                            f"Exportando MP4: {pct}%  ({idx+1}/{total_f})")
                        prog_win.update(); self.root.update()

            # ── Éxito ─────────────────────────────────────────
            elapsed_total = time.perf_counter() - t0_exp
            prog_bar['value'] = total_f
            lbl_frame.config(
                text=f"✅  Completado — {total_f} frames en {elapsed_total:.0f}s",
                fg=CG)
            lbl_eta.config(text=f"📁  {fp}", fg=CW)
            self._status_var.set(f"✓ Guardado: {os.path.basename(fp)}")
            prog_win.after(3500, prog_win.destroy)

        except Exception as e:
            lbl_frame.config(text=f"⚠  Error al exportar", fg=CR)
            lbl_eta.config(text=str(e)[:80], fg=CR)
            self._status_var.set(f"⚠ Error: {e}")
            prog_win.after(5000, prog_win.destroy)
        finally:
            plt.close(fig_ex)
